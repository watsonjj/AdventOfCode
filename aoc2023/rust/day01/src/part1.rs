...
const PATTERN: &str = r"\d|one|two|three|four|five|six|seven|eight|nine";


/// Extract a digit from the text following the regex pattern.
/// Returns "0" if not match found.
fn extract_digit_str<'a>(re: &Regex, text: &'a str) -> &'a str {
...
fn extract_trebuchet_calibration_value(calibration_line: &str) -> u32 {
    // Define patterns
    static P0: Lazy<String> = Lazy::new(|| format!(r"({PATTERN})"));
    static P1: Lazy<String> = Lazy::new(|| format!(r".*({PATTERN}).*?$"));
    static RE0: Lazy<Regex> = Lazy::new(|| Regex::new(&P0).unwrap());
    static RE1: Lazy<Regex> = Lazy::new(|| Regex::new(&P1).unwrap());

    // Extract matches
    let digit_str_0 = extract_digit_str(&RE0, calibration_line);
    let digit_str_1 = extract_digit_str(&RE1, calibration_line);
...
pub fn extract_trebuchet_calibration<'a>(
    ...
    calibration_document
        .lines()
        .map(move |l| extract_trebuchet_calibration_value(l))
}
